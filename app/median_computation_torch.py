...
def optim(metric, n, logits, rv_y=None, std_dev=10, reg=0.01, steps=10000, use_mlflow=False):
    # Source distribution
    ...
    # Optimization
    optimizer = torch.optim.SGD([s], lr=0.001, momentum=0.9)
    ...
    for t in range(steps):
        ...
        loss = smoothed_metric(y, s) + reg * torch.linalg.norm(s, dim=-1, ord=2) ** 2
        loss.backward()
        optimizer.step()
        scheduler.step()
        ...
        if t % (steps / 200) == 0:
            ...
            current_score_loss = []
            logger.info(f"s = {s} and sigma = {argsort(s, descending=True)}")
            l = 0
            l_opt = 0
            M = 1000
            for i in range(M):
    ...
    logger.info(f"Optimal score = {s}")
    return losses, score_losses, s
...
def compute_median(config):
    ...
    # Smoothing
    losses, score_losses, s = optim(metric, config.n, config.logits, rv_y=None, std_dev=config.std_dev, reg=config.reg, steps=config.steps)
...
if __name__ == "__main__":
    ...
    if my_config.use_mlflow:
        with mlflow.start_run(run_name="Median computation SGD", nested=True):
            ...
            s = compute_median(my_config)
            log_metric_mlflow("Optimal score", s, use_mlflow = my_config.use_mlflow)
    else:
