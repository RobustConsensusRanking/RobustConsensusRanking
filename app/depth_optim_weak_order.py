...
def depth_optim(epsilon, metric, n, logits, rv_y=None, std_dev=10, reg=0.01, steps=10000, use_mlflow=False):
    ...
    for t in range(steps):
        ...
        rv_score = BucketRk(weak_order)

        optimizer.zero_grad()
        y = rv_y.sample().to(device).type(default_tensor_type)
        sample_s = rv_score.sample().to(device).type(default_tensor_type)

        depth_source = smoothed_metric(y, s)
        depth_score = smoothed_metric(sample_s, s)

        loss = -torch.abs(depth_source - depth_score) + reg * torch.linalg.norm(s, dim=-1, ord=2) ** 2
        loss.backward()
        optimizer.step()
        scheduler.step()
...
if __name__ == "__main__":
    ...
    if my_config.use_mlflow:
        with mlflow.start_run(run_name="Depth minimization SGD", nested=True):
